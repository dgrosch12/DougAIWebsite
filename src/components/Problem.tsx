        ...
        <div className="mt-16 grid gap-6 md:grid-cols-3">
          {problems.map((problem, i) => (
            <AnimatedSection key={i} delay={i * 0.15}>
              <div className="group relative rounded-2xl border border-[#FF4444]/20 bg-surface/50 p-8 transition-all hover:border-[#FF4444]/40 hover:bg-surface">
                {/* Warning glow */}
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-b from-[#FF4444]/[0.03] to-transparent pointer-events-none" />

                <div className="relative">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#FF4444]/10 text-[#FF4444]">
                    {problem.icon}
                  </div>
                  <h3 className="mt-5 text-xl font-bold text-white">
                    {problem.headline}
                  </h3>
                  <p className="mt-3 text-[#E8E8ED]/50 leading-relaxed">
                    {problem.description}
                  </p>
                </div>
              </div>
