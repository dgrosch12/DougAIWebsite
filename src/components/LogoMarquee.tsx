...
const tools: Tool[] = [
  ...
  },
  {
    name: "OpenAI",
    logo: (
      <svg viewBox="0 0 24 24" fill="#202123" className="h-9 w-auto">
  ...
  },
  {
    name: "Next.js",
    logo: (
      <svg viewBox="0 0 24 24" fill="#000000" className="h-9 w-auto">
...
function LogoItem({ tool }: { tool: Tool }) {
  return (
    <div className="inline-flex items-center gap-3 px-6 flex-shrink-0">
      <span className="flex-shrink-0">{tool.logo}</span>
      <span className="text-base font-semibold text-heading/70 whitespace-nowrap">
        {tool.name}
      </span>
    </div>
  );
